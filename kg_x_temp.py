...
def prepare_kgs_data(subgraph_nodes, subgraph_edges):
    """
    Prepares data in the .kgs format (zlib compressed JSON), ensuring nodes and edges
    have formatting applied for consistent visualization.
    """
    # 1. Assign distinct colors to nodes based on their group/type
    ...
    # 2. Apply default font styling to all nodes
    for node in subgraph_nodes:
        node['font'] = {"color": "white", "strokeWidth": 2, "strokeColor": "black"}

    # 3. Manually build clean edge objects with font styling
    formatted_edges = []
    for edge in subgraph_edges:
        ...
        if "color" in edge:
            clean_edge["color"] = edge.get("color")
        
        formatted_edges.append(clean_edge)

    # 4. Build the final state object
    state = {
        "type": "IsolationState",
        "version": 1,
    ...
    json_string = json.dumps(state, indent=2, allow_nan=False)
